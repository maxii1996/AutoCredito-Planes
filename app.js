...
function renderPriceTabs() {
  ...
  if (subtitle) subtitle.textContent = active?.locked ? 'Pestaña original' : 'Pestaña personalizada';
  if (renameBtn) renameBtn.disabled = !active || active.locked;
  if (deleteBtn) deleteBtn.disabled = !active || active.locked || priceTabs.length <= 1;
  if (!list) return;
  list.innerHTML = priceTabs.map(tab => `
    <button class="tab-chip ${tab.id === activePriceTabId ? 'active' : ''}" data-tab="${tab.id}">
      <span>${tab.label}</span>
      ${tab.locked ? '<span class="pill mini">Original</span>' : ''}
    </button>
  `).join('');
...
function openPriceTabModal(mode = 'create') {
  ...
  if (baseSelect) {
    baseSelect.innerHTML = priceTabs.map(tab => `<option value="${tab.id}">${tab.label}${tab.locked ? ' (Original)' : ''}</option>`).join('');
...
function updatePlanSummary() {
  ...
  const ribbon = document.getElementById('planTimelineRibbon');
  if (ribbon) {
    const ribbonSteps = [
      {
        title: 'Integración objetivo',
      ...
      },
      {
        ...
        detail: `Saldo en plan ${currency.format(projection.outstanding)} · Cuota pura ${currency.format(projection.cuotaPura || 0)}`,
        chips: [`Inicio en cuota ${firstPayable}`, coveredText],
        kind: 'pending'
      }
    ];
    ribbon.innerHTML = ribbonSteps.map(step => `
      <div class="ribbon-step" data-kind="${step.kind}">
        <div class="ribbon-label"><span class="dot"></span>${step.title}</div>
        <p class="ribbon-body">${step.detail}</p>
        <div class="ribbon-meta">${step.chips.filter(Boolean).map(c => `<span class="pill">${c}</span>`).join('')}</div>
      </div>
    `).join('');
  }
